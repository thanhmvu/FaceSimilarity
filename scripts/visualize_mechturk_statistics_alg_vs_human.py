...
SHOW_INFO = False
output_file = "../results/second_experiment_results/" + DATASET + "_" + MODEL_NAME_NO_EXT + "_" + str(threshold) + ".html"
...
dist=np.loadtxt("../distance_mat/" + file_name.replace(".","_distmat_" + MODEL_NAME_NO_EXT + "_."))
print('Loaded ' + "../distance_mat/" + file_name.replace(".","_distmat_" + MODEL_NAME_NO_EXT + "_."))
distsq=squareform(dist)
print('Finished processing ' + "../distance_mat/" + file_name.replace(".","_distmat_" + MODEL_NAME_NO_EXT + "_."))
    ...
    for row in reader:
        if is_header :
          ...
          for i in range(10):
            ...
            first_pair_score = getDistance(first_pair_first_image_name, first_pair_second_image_name)
            second_pair_score = getDistance(second_pair_first_image_name, second_pair_second_image_name)
...
for first_pair_name, second_pairs in pairs_results.iteritems():
  for second_pair_name, vote in second_pairs.iteritems():
    ...
    if vote == 1 and computer_vote == 0 and agreement_pecentage >= threshold :
...
with file(output_file, 'w') as modified: 
  modified.write("<h2>Tests with more than " + str(threshold) + " agreement where the algorithm disagreed with voters (" + str(total_tests) + " tests out of  = )</h2>\n")
  if SHOW_INFO:
  ...
  modified.write("<br><h2>100 examples of failed comparisons : </h2>\n")
  modified.write("<br>\n")
  modified.write("The <span style='background:#7ade7a'>green pair</span> is the one voters chose as more similar <br> \n")
  modified.write("The <span style='background:#ffffff'>white pair</span> is the one the algorithm chose as more similar <br> \n" + data)
